...
function Inspector( props, allPages ) {
	...
	let pageListOptions = [
		{ value: 0, label: __( 'Select a parent page', 'ctx-blocks' ) },
	];
	...
	const categoryList = useSelect(
		( select ) => {
			...
			const categoryListOptions = [
				{ value: 0, label: __( 'Select a category', 'ctx-blocks' ) },
			];
			if ( result == null ) {
				result = [];
			}
		...
		<InspectorControls>
			...
			>
				<SelectControl
					...
					options={ [
						...
						{
							value: 'categories',
							label: __( 'Categories', 'ctx-blocks' ),
						},
					] }
				/>
				...
				{ dataType === 'posts' && (
					<SelectControl
						label={ __(
							"Category (including it's children)",
							'ctx-blocks'
						) }
				...
				<SelectControl
					...
					options={ [
						{
							value: 'desc',
							label: __( 'Descending', 'ctx-blocks' ),
						},
						{
							value: 'asc',
							label: __( 'Ascending', 'ctx-blocks' ),
						},
					] }
				/>
				...
				<RangeControl
					...
					min={ 1 }
					help={ __(
						'How meny items should be displayed?',
						'ctx-blocks'
					) }
					onChange={ ( value ) => {
			...
			>
				...
				<PanelRow>
					<ToggleControl
						label={ __(
							'Highlight currently loaded menu item',
							'ctx-blocks'
						) }
						checked={ showActive }
						onChange={ ( value ) =>
