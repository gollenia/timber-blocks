...
import { Button, Icon, Toolbar, ToolbarItem } from '@wordpress/components';
...
export default function Edit({ ...props }) {
	...
	const Image = () => {
		...
		if (image.subtype == 'svg+xml')
			return (
				<figure>
					<img width={`${width}%`} src={image.url} />
				</figure>
			);
		if (!image.sizes)
			return <img className={imageClasses} src={image.url} />;
		...
		if (image.sizes.medium)
			return (
				<picture>
					<div className="ctx:image__warning">
						<label>
							<Icon icon="warning" />
							<span>
								{__('The image may be too small', 'ctx-blocks')}
							</span>
						</label>
					</div>
					<img
		...
		if (image.sizes.thumbnail)
			return (
				<picture>
					<div className="ctx:image__warning ctx:image__warning--fatal">
						<label>
							<Icon icon="warning" />
							<span>
								{__('The image is too small', 'ctx-blocks')}
							</span>
						</label>
					</div>
					<img
